...
> 上传、识别、歌词处理、ASS 字幕、视频合成和部署脚本这条处理闭环来自原项目。本 fork 重写了歌词对齐，接入了强制对齐模型，并补上了原项目列为“后续开发”的时间轴核对、字幕预览和样式编辑等功能。详见[与原项目的差异](#与原项目的差异)。
...
- **字幕样式**：默认折叠，点“自定义样式”展开；也可以等出了成片再换。

### 2. 核对时间轴

任务停在“等待核对”时，任务页会直接显示核对界面；已完成的任务点“核对并修正时间轴”进入同一个界面。

- **看**：播放原视频，下方预览条用浏览器实时绘制变色效果，不需要等渲染。可以切到“仅人声”来听，更容易判断每个字的起点。
...
| 时间精度 | 只依赖 Whisper 的词级时间戳 | 可选接入 karatimer 对整首歌做 CTC 强制对齐，不再需要语音识别；也可用 Qwen3-ForcedAligner 分段精修 |
| 间奏 | 歌词可能被对到没人唱的间奏里 | 可选用 Roformer 分离出干净人声，检测间奏并把落在里面的句子自动挪出来 |
| 读音 | pykakasi 逐字查词典，带送假名的动词、形容词常被读成音读 | 本机读音改用 OpenJTalk 形态分析；读音可以在核对界面里逐词修改 |
| 素材 | 只能上传本地文件；歌词只接受纯文本 | 可直接填 YouTube 链接；可直接粘贴 LRC |
| 人工核对 | 无，需导出文件用外部工具修改 | 内置核对界面：原视频 + 实时变色预览、波形拖动、打点、改读音、AI 重新对齐单句 |
| 字幕样式 | 固定样式 | 配色、字体、字号、位置、排布、发光、注音开关；完成后可只换样式重新生成 |
| 成片 | 需要在 ON / OFF VOCAL 之间二选一，想要两版得跑两次 | 一次生成两个版本，字幕只烧录一次 |
| 首页 | 左侧大段介绍；只能靠网址回到某个任务 | “新建任务 + 最近任务列表”的工具式布局 |
