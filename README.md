...
| 模块 | 能力 |
|---|---|
| 素材来源 | 上传本地 MP4（校验、1 GB 限制、上传进度），或填写视频链接由本机用 yt-dlp 下载到任务目录（默认只允许 YouTube）；歌词粘贴或 UTF-8 TXT 上传 |
...
| 核对与修正 | 合成前暂停等待核对；逐句播放、修改起止时间、打点；AI 重新对齐单句；手动修改不会被后续自动对齐覆盖 |
...
- 启用了强制对齐时，可以让 AI 重新对齐改过的句子：只需把起止范围大致标对，模型会在这个范围内重新精确到每个词（每次约 1 分钟，主要花在加载模型上，建议多改几句再一起执行）
...
当前基线：后端 179 项测试；前端 8 个测试文件、41 项测试。类型检查中 `frontend/worker/index.ts` 有两个缺少 Cloudflare 类型定义的既有报错，与功能无关。

## 已知限制

- 核对界面只能调整整句的起止时间（可在波形时间轴上拖动），不能逐字调整。
- 还没有假名注音（读音）的检查与修正界面。读音错误会同时影响屏幕上的注音和对齐用的音节序列。
- 必须提供歌词；没有无歌词模式，也不支持导入 LRC、ASS 等已有的字幕文件。
...
- 读音与注音的检查和修正
- 导入带逐句时间的 LRC 歌词，跳过 Whisper 粗定位，直接做强制对齐
