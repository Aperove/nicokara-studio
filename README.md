...
| **界面** | 明亮 / 暗色 / 跟随系统三种外观，右上角切换并记住选择；首页、任务页和核对工作台同一个宽度 |
...
核对界面是一个铺满窗口的工作台（顶栏以下）：左边是视频、变色预览和波形，始终可见；右边是歌词列表，单独滚动并自动跟随正在唱的句子；字幕样式从右侧滑出。首页、任务页和工作台用的是同一个页面宽度。窗口很窄时（小于约 1000×480）工作台会自动改成单列。

- **看**：播放原视频，下方预览条用浏览器实时绘制变色效果，不需要等渲染。预览按这个任务的字幕样式绘制（配色、字体、描边、发光、注音），太长的句子会缩小到放得下。
- **调样式**：点“字幕样式”打开样式面板，改动会立刻反映在预览条上，并在点“确认并合成视频”时一并保存。
- **看真实画面**：样式面板打开时，左边的播放器会换成一张**真实画面**：用和最终视频同一个渲染器，在这首歌的视频上画出来的一帧，字号、位置、自动缩小和黑边都和成片一致，每次改样式后约半秒刷新。可以看“最长的一句”（它决定整首歌缩到多小），也可以看当前播放位置。任务页的“修改字幕样式”里也有同样的画面。可以切到“仅人声”来听，更容易判断每个字的起点。
- **空格键**随时播放/暂停，不管刚才点过或拖过什么（正在文字框里输入时除外）。
...
- **改读音**：歌词列表里，带汉字的词上方直接显示它的假名读音，读错的一眼就能看到；点这个词就能改（也可以用每句的“读音”按钮列出整句的词）。读音错了，屏幕上的注音和句内的变色节奏都会跟着错。注意词是按分析器的结果切分的，动词词干和后面的助词是两个词，只需填汉字那一部分的读音。
...
字号只会自动**缩小**，不会放大：你选的字号是上限。整首歌统一用一个字号，由最宽的一句决定，最小缩到 40。“左右交错”排布时每句以画面 35% / 65% 处为中心，能放下的字数比“居中”少约三成，句子偏长时选“居中”缩得更少。

能放多少字取决于字体：同样的字号，BIZ UD 和 MS 系列的字和字号一样宽，Noto、游ゴシック、メイリオ只有字号的七成左右。排版时用的是本机安装的那个字体的真实字宽，所以注音会对准它下面的汉字，句子也不会超出画面。
...
| `POST /{id}/preview` | 用给定的样式渲染一帧真实画面（JPEG），不保存样式；不给时间时取最长的一句 |
...
当前基线：后端 215 项测试；前端 8 个测试文件、43 项测试。类型检查中 `frontend/worker/index.ts` 有两个缺少 Cloudflare 类型定义的既有报错，与功能无关。
...
- 字幕样式不能保存为个人预设；描边粗细等更细的参数还不能调。新建任务时还没有视频可画，所以那里的样式设置没有画面预览。
- 字体由负责渲染的那台机器提供；界面里的字体列表是 Windows 常见日文字体，Docker 部署中只有 Noto Sans CJK 可用。选中的字体在渲染的机器上没有安装时，会换成一个已安装的日文字体（依次尝试 Noto Sans CJK JP、Noto Sans JP、游ゴシック、Hiragino Sans、メイリオ、MS ゴシック），并按它的字宽排版；一个都找不到时才退回估算。
